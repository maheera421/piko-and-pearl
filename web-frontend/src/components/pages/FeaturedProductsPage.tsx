...
export function FeaturedProductsPage({ onNavigate, products: propProducts, categories }: FeaturedProductsPageProps) {
  ...
  const getWishlistId = (p: any, categoryName?: string) => {
    if (!p) return '';
    if (p._id) return p._id;
    const cat = (categoryName || p.category || '').toString();
    return `${cat}-${p.id}`;
  };
  ...
  const toggleWishlist = (product: any, category: string) => {
    const wishlistId = (product && (product._id || product.id)) ? (product._id || `${category}-${product.id}`) : '';
    const wishlistItem = {
      id: wishlistId,
      name: product.name,
      price: product.price,
    ...
    const wasInWishlist = isInWishlist(wishlistId);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Removed search and filters per request — featured page shows only header and products */}
